...
`spatialdata_io.xenium()` auto-loads the H&E when `he_image.ome.tif` and
`he_imagealignment.csv` are present in the Xenium directory following the expected
naming convention. When the image lives at an arbitrary path, pass `he_image` and
`he_alignment` as notebook params to load it explicitly.

```{python}
if "he_image" not in sdata.images and he_image and he_alignment:
    ...
    print(f"Loaded H&E from {he_image}")
