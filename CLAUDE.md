...
All three also take the **filtering cut** as `--min_counts` (cells below that many transcripts are dropped) and `--min_cells` (genes detected in fewer cells are dropped), following the same null-defers-to-the-script convention as `--resolutions`, so the defaults of 10 and 5 live only in `bin/cluster_sdata*.py`. Choose them by reading `qc_report`'s retention curves — that deck exists for this decision and nothing else consumes it. Two wiring details: `main.nf` tests `params.min_counts != null` rather than using `?:`, because `0` is a meaningful threshold (keep every cell) that Groovy truthiness would silently collapse back to the default; and `cluster_sdata_gpu_ooc` applies the same numbers as a boolean mask over `obs["total_counts"]` / `var["n_cells_by_counts"]` rather than through `filter_cells`/`filter_genes`, since filtering through views is incompatible with Dask-backed X — the flags mean the same thing but reach the data by a different route.
...
nextflow run main.nf --step cluster_sdata --samplesheet my_sample_zarrs.csv --min_counts 25 --min_cells 3
