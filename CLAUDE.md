...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    ...
    Three slides per sample and nothing else: per-cell QC histograms (transcripts, genes, cell area, nucleus area, all log, median marked, zeros counted in the panel title since a log axis cannot draw them), transcripts against genes detected as one point per cell (the joint view the margins cannot give — a cell with many transcripts over few genes sits off the main arm while looking ordinary in either histogram), and transcripts/genes in tissue coordinates on log colour scales. There is no cohort section and the deck's only output is the pptx — it reports what the data looks like and stops, so choosing a threshold means reading the slides and setting the cut in `cluster_sdata*` yourself. The tissue slide picks its panel layout per sample: two wide-strip ROIs side by side make a 6:1 figure that pandoc shrinks to a centimetre-tall band, so whichever of 1x2 / 2x1 lands closest to the slide's aspect wins
