...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    ...
    **Molecule maps are the deck's only spatial view not conditioned on segmentation**, and that is their reason to exist: every tissue slide above is drawn from cells, so a molecule assigned to no cell is invisible there. `transcript_map_slide` bins the points into a raster — density (log) with no mask, or a mask's **fraction per bin** with one. Fraction, not count, because a raw count of bad molecules just retraces tissue density. Three masks (`TX_FRACTION_MAPS`): below qv, assigned to no cell, and control-or-unassigned codeword. On a test ROI the second is **5.3% of all transcripts**, a segmentation number nothing else in the pipeline reports, and it concentrates at the tissue edge and in scattered gaps.
    - `BIN_UM` is a bin width in **microns, not a bin count**, so a bin means the same thing on a 2,000 µm ROI and on a whole slide and two samples can be read against each other.
