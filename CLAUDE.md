...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    ...
    **Molecule maps are the deck's only spatial view not conditioned on segmentation**, and that is their reason to exist: every tissue slide above is drawn from cells, so a molecule assigned to no cell is invisible there. `transcript_map_slide` bins the points into a raster — density (log) with no mask, or a mask's **fraction per bin** with one. Fraction, not count, because a raw count of bad molecules just retraces tissue density. Three masks (`TX_FRACTION_MAPS`): below qv, assigned to no cell, and control-or-unassigned codeword. On a test ROI the second is **5.3% of all transcripts**, a segmentation number nothing else in the pipeline reports, and it concentrates at the tissue edge and in scattered gaps.
    - `BIN_UM` is a bin width in **microns, not a bin count**, so a bin means the same thing on a 2,000 µm ROI and on a whole slide and two samples can be read against each other. It is **5 µm — sub-cellular**, against a median cell about 7.5 µm across, and is set from how many molecules a bin can afford to lose rather than from anatomy. Xenium's density makes the limit far finer than it looks: a 5 µm bin still holds a median ~110-130 transcripts and blanks only 1-6% of occupied bins, while 3 µm blanks 12-22% and 2 µm blanks half. Do not coarsen it back without measuring — at 25 µm a bin held ~3,000 transcripts, ~25x more than needed to be stable, and the maps showed no anatomy at all; at 5 µm follicles, stromal texture and the segmentation rings around antral structures are all legible.
    - `MIN_BIN_TRANSCRIPTS` blanks sparse bins. A bin holding two molecules of which one is bad is not 50% bad — it is empty space, and on a fraction map those bins are the loudest thing on the slide.
    - **These axes are in µm**, unlike every other spatial slide here, because the points element is stored in µm and carries a 4.7059 scale **to** the pixel `global` frame rather than from it. So this is the one place the deck can draw a **scale bar** — `draw_scale_bar`, white with a dark outline because it sits on a full-range colormap and any single ink is invisible on either the dark end or the light one. This does not solve the scale bar for the label-based slides, which still need a pixel size `create_sdata` does not record.
