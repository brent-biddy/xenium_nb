...
- `annotate_sdata` — labels each Leiden cluster with a cell type from a marker YAML (`--markers assets/ovarian_markers_human.yaml`, required — the file is species- and tissue-specific, so there is no safe default). Scores every cell against each cell type's marker set with `sc.tl.score_genes`, averages per cluster, and writes one `cell_type_res_<r>` column beside each `leiden_res_<r>`, plus per-cell `score_<cell_type>` columns and a `annotate_sdata_scores.tsv` audit table. Like the `cluster_sdata*` steps it defaults to **every** resolution present rather than committing to one; `--resolutions` narrows it. Three things are easy to get wrong:
    - **Raw `score_genes` values are not comparable between gene sets.** A set whose markers are broadly expressed scores high in every cluster and wins them all — on a real ROI, `fibroblast` took 8 of 9 clusters this way. So the default (`--scale zscore`) standardizes each cell type's cluster means **across clusters** before taking the per-cluster argmax, turning "which set has the biggest number" into "which type is this cluster most enriched for relative to the others". `--scale none` restores the raw ranking
    - **The z-score cannot say whether a cluster is enriched at all** — it is centred by construction, so every cell type wins somewhere and every cluster has a winner. The winner is therefore chosen by z but gated on its **raw** mean: at or below `--min_score` (default 0, i.e. no enrichment over `score_genes`' expression-matched background) the cluster becomes `unassigned`. A weakly-positive spurious winner still passes, so a cell type absent from the sample can still be labelled
    - **Marker sets are intersected with the panel first**, because `score_genes` drops missing genes silently. Sets left under `--min_genes` (default 3) are skipped and reported. On a 5K panel this skips 8 of the 21 human ovary types, including `leukocyte` and `neuron` at 0 genes — labels are only ever drawn from what survived
...
nextflow run main.nf --step annotate_sdata --samplesheet results/cluster_sdata_samplesheet.csv --markers assets/ovarian_markers_human.yaml
...
nextflow run main.nf --step annotate_sdata -stub --samplesheet assets/ci_analyze_samplesheet.csv --markers assets/ovarian_markers_human.yaml
