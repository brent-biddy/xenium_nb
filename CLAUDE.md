...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    - **It never touches X.** Every metric it shows is already in `obs` from `create_sdata`, so it reads `obs` and `obsm["spatial"]` via `ad.io.read_elem` and leaves the count matrix on disk. This is the payoff `create_sdata` was annotating QC metrics for
    ...
    - **It derives nothing, and has no fallbacks.** Every obs column it reads is listed in `REQUIRED_OBS` and checked up front in `load_sample`, so a zarr predating a column fails on the first sample with that column named rather than several slides later. A fallback derivation would be a second definition of the metric, letting a stale cohort render a deck whose numbers came from outside the pipeline — re-run `create_sdata` instead

    Two slides per sample and nothing else: per-cell QC histograms (transcripts, genes, cell area, nucleus area, all log, median marked, zeros counted in the panel title since a log axis cannot draw them), and transcripts/genes in tissue coordinates on log colour scales. There is no cohort section and the deck's only output is the pptx — it reports what the data looks like and stops, so choosing a threshold means reading the slides and setting the cut in `cluster_sdata*` yourself. The tissue slide picks its panel layout per sample: two wide-strip ROIs side by side make a 6:1 figure that pandoc shrinks to a centimetre-tall band, so whichever of 1x2 / 2x1 lands closest to the slide's aspect wins
