...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    - **It never touches X.** Every metric it shows is already in `obs`/`var` from `create_sdata`, so it reads `obs`, `var`, and `obsm["spatial"]` via `ad.io.read_elem` and leaves the count matrix on disk. This is the payoff `create_sdata` was annotating QC metrics for
    - **It must run on `create_sdata` output, not `cluster_sdata`'s.** Clustered zarrs have already had the `min_counts` cut applied, so the discarded tail — the thing the deck exists to size — is gone, and every threshold looks harmless
    - **The negative-control fraction is derived here**, from the five control/codeword counters, because no upstream step emits one yet; it reads `obs["pct_control"]` first if a future `create_sdata` adds it. The denominator is rebuilt as `transcript_counts + controls` rather than read from `total_counts`, since `cluster_sdata` overwrites `total_counts` with `X.sum(axis=1)` and the column therefore means different things before and after clustering. Per-cell control rates are near zero on real data (median 0, ~87% of cells carry none), so controls are shown as sample-level totals per category on a log axis rather than as a per-cell distribution, which would be a flat line

    Per sample it emits per-cell QC histograms, transcripts and genes in tissue coordinates, a cell-retention-vs-threshold curve plus a table of selected cuts, control and segmentation-method composition, and gene detection vs the `min_cells` cut. The cohort section follows at the end: a summary table, per-cell ECDFs overlaid across samples, and both retention curves overlaid. The tissue slide picks its panel layout per sample — two wide-strip ROIs side by side make a 6:1 figure that pandoc shrinks to a centimetre-tall band, so whichever of 1x2 / 2x1 lands closest to the slide's aspect wins. Note pandoc's pptx writer puts every table on a slide of its own, so each table needs its own `##` heading or it lands on an untitled slide named after its first header cell.

    It also writes `qc_thresholds.csv` (`sample,min_counts,min_cells`) beside the deck, pre-filled with the pipeline defaults. Nothing infers a threshold from the curves — the file saves the typing and the sample-name spelling, and the values are meant to be edited by hand. **No step consumes it yet**: `cluster_sdata*` still hardcode `min_counts=10` / `min_cells=5`, so acting on the deck's conclusion currently means editing those scripts
