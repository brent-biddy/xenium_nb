...
- `qc_report` — renders `qc_report.qmd` into one cohort-level pptx deck over every **raw `create_sdata` zarr**, to be read *before* clustering so the filtering thresholds are chosen from the data. Same staging-is-the-contract fan-in as `cluster_report` (indexed `stageAs`, glob `*.zarr`, no params, no registry entry, pinned `jupyter: python_spatial`), and the same `error: true` caveat — read the slides, not the exit status. Three things are specific to it:
    ...
    Five slides per sample and nothing else: per-cell QC histograms (transcripts, genes, cell area, nucleus area, all log, median marked); the nucleus-to-cell area ratio on its own slide (a bounded ratio wants a linear axis and must not inherit the log panels' zero-dropping, which is why it is not a fifth histogram panel); and transcripts against genes detected as one point per cell, repeated three times coloured by `transcripts_per_gene`, `cell_area`, and `nucleus_area` — the joint view the margins cannot give, since a cell with many transcripts over few genes sits off the main arm while looking ordinary in either histogram. Then the segmentation in tissue coordinates, coloured by transcripts and by genes on log scales. There is no cohort section and the deck's only output is the pptx — it reports what the data looks like and stops, so choosing a threshold means reading the slides and setting the cut in `cluster_sdata*` yourself.

    Two details on the histograms are worth keeping straight. Cells fall off a log panel two ways and are counted separately: a **zero** cannot be drawn, and a value that was **never measured** is NaN — the reader leaves `nucleus_area` NaN, not 0, for a cell segmented without a nucleus (70 of 21,724 on one ROI, matching `nucleus_count == 0`). Folding those together, or filtering non-finite values before counting, silently loses exactly the cells a QC deck exists to surface.

    The tissue slide uses **`spatialdata_plot`'s `render_labels`**, not a scatter, so each cell is painted at its own segmented footprint. That is what retires the marker-size question — no constant travels between a 2,000 µm ROI and a whole slide, and deriving one from cell spacing still leaves a coverage factor that is pure taste. `render_labels` rather than `render_shapes` because the table annotates `cell_labels`, so it needs no schema change, while colouring `cell_boundaries` would need a second table keyed on `cell_id`; at this magnification a cell is ~2 pt across, so the polygons' extra fidelity is invisible anyway. Three traps here: **`LogNorm` fails** (`spatialdata_plot` calls `norm.autoscale_None` on the 2D label array and matplotlib 3.11's generated `LogNorm` transforms it unconditionally), hence the `SafeLogNorm` shim — delete it when the libraries agree; spatialdata's `global` frame for a Xenium read is in **pixels**, so the axes are tick-less rather than mislabelled as µm, and the squidpy version's scale bar is gone with it; and the slide is called **once per panel** so each metric gets its own norm and colourbar. The layout is chosen per sample — two wide-strip ROIs side by side make a 6:1 figure that pandoc shrinks to a centimetre-tall band, so whichever of 1x2 / 2x1 lands closest to the slide's aspect wins
